...
export async function PUT(req: Request) {
    try {
        ...
        const idx_checkList = boardJson.data[idx_b].lists[idx_lb].cards[idx_clb].CheckList.findIndex(ch => ch.id === checkList.ch_id);
        if (idx_checkList == -1) {
            return NextResponse.json({ message: 'Cannot find checklist' }, {status: 404});
        }
        boardJson.data[idx_b].lists[idx_lb].cards[idx_clb].CheckList[idx_checkList].show = checkList.check;
